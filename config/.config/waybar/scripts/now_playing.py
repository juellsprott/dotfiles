...
class PlayerManager:
    ...
    def write_output(self, text, player):
        ...
        # If the new text is shorter than max_length, pad it with spaces
        if len(text) < self.max_length:
            text = text.ljust(self.max_length)

        # Check if scrolling is needed (text longer than max_length)
        if len(text) > self.max_length:
            self.start_scrolling(text, output)
        else:
            # Stop any existing scrolling thread
            self.stop_scrolling()
    ...
    def start_scrolling(self, text, output):
        # Stop any existing scrolling thread
        self.stop_scrolling()
        ...
        def scroll_text():
            scrolled_text = text + " "
            while getattr(self.scroll_thread, "running", True):
                scrolled_text = scrolled_text[1:] + scrolled_text[0]
                output["text"] = scrolled_text[:self.max_length]
                sys.stdout.write(json.dumps(output) + "\n")
                sys.stdout.flush()
                time.sleep(self.scroll_delay)

        self.scroll_thread = threading.Thread(target=scroll_text)
        self.scroll_thread.running = True
        self.scroll_thread.start()

    def stop_scrolling(self):
        if self.scroll_thread and self.scroll_thread.is_alive():
            self.scroll_thread.running = False
