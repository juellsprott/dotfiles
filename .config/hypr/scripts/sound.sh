...
get_volume_icon () {
    ...
    if ((0==$1))
    then
        echo "/usr/share/icons/Numix/22/status/audio-volume-muted.svg"
    elif ((1<=$1 && $1<=33))
    then
        echo "/usr/share/icons/Numix/22/status/audio-volume-low.svg"
    elif ((34<=$1 && $1<=66))
    then
        echo "/usr/share/icons/Numix/22/status/audio-volume-medium.svg"
    else
        echo "/usr/share/icons/Numix/22/status/audio-volume-high.svg"
    fi
}
...
switch_audio_sink() {
    ...
    SINK_INFO=$(pactl list sinks | grep -E "Description|object.serial" | sed "s/\s*Description: \(.*\)\|\s*object.serial = \"\(.*\)\"/\1\2/")
...
show_notification () {
    ...
    spaces=$(seq -s "+" 0 $((24 - (volume / 4) )) | sed 's/[0-9]//g')
    dunstify --appname="Volume" -i $ICON -t 1000 -r 5555 -u normal "${bar} $volume %"
